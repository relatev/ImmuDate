...
public class GregorianChronology extends Chronology {
    ...
    public int getDaysOfMonth(Month month) {
        if(isLeapYear(month.SUPERIOR))
            return MAX_DAYS_A_MONTH[(int) month.VALUE - ZERO.MONTH_ZERO];
        else
            return MIN_DAYS_A_MONTH[(int) month.VALUE - ZERO.MONTH_ZERO];
    }
